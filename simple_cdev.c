...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Me");
MODULE_DESCRIPTION("A simple char device backed by /proc");
MODULE_VERSION("0.1");
...
static struct file_operations scdev_proc_fops =
{
	.owner = THIS_MODULE,
	.write = scdev_proc_write,
	.read = scdev_proc_read,
};

/* Stuff related to char dev*/
static int scdev_open(struct inode *inodep, struct file *filep)
{
	printk(KERN_DEBUG "scdev: opened\n");
	return 0;
}

static int scdev_release(struct inode *inodep, struct file *filep)
{
	printk(KERN_DEBUG "scdev: released\n");
	return 0;
}
...
static struct file_operations scdev_fops =
{
	.open = scdev_open,
	.read = scdev_read,
	.write = scdev_write,
...
static int __init simple_cdev_init(void)
{
	...
	printk(KERN_DEBUG "Simple char device got major %d\n", scdev_major);
	...
	printk(KERN_DEBUG "Simple char device class created\n");
	...
	/* init /proc */
	proc_entry = proc_create(DEVICE_NAME, 0666, NULL, &scdev_proc_fops);
	printk(KERN_DEBUG "Simple char device has been created\n");
...
static void __exit simple_cdev_exit(void)
{
	...
	printk(KERN_DEBUG "Simple char device exited\n");
}
